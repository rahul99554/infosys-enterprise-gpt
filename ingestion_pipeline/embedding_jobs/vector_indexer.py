...
class EnterprisePDFIndexer:
    def __init__(self, data_root_path: str = None, vector_db_path: str = None, google_api_key: str = None):
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.abspath(os.path.join(script_dir, "..", ".."))
        self.data_root = data_root_path or os.path.join(project_root, "data")
        self.vector_db_path = vector_db_path or os.path.join(project_root, "vector_db")

        api_key = google_api_key or os.getenv("GOOGLE_API_KEY")
        if not api_key:
            raise ValueError("GOOGLE_API_KEY not found. Please set your environment variable.")

        print("Initializing Embedding Model (Google Generative AI)...")
